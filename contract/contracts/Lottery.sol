...
contract Lottery {
    ...
    function fulfillRandomness(uint256 externalData) external {
        ...
        require(betPlaced && !resolved, "Nessuna scommessa attiva da risolvere");
        // Generazione dei numeri casuali (range 0-99) basata sul dato esterno e sul timestamp
        drawnNum1 = uint256(keccak256(abi.encode(externalData, block.timestamp, 1))) % 100;
        drawnNum2 = uint256(keccak256(abi.encode(externalData, block.timestamp, 2))) % 100;
        bool win = (drawnNum1 == chosenNum1 && drawnNum2 == chosenNum2);
        if(win) {
    ...
    ) 
    {
        return (
            ...
            betAmount,
            player,
            (drawnNum1 == chosenNum1 && drawnNum2 == chosenNum2)
        );
    ...
    function resetLottery() public {
        require(resolved, "La lotteria non e' ancora risolta");
        betPlaced = false;
        resolved = false;
        player = address(0);
        betAmount = 0;
